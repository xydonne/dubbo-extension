...
public class RestExpressInvoker<T> extends AbstractInvoker<T> {
    ...
    protected Result doInvoke(Invocation invocation) throws Throwable {
        ...
        try {
            byte[] response = post(urlStr + "/" + invocation.getMethodName(), request, RpcContext.getContext().getAttachments());
            Class<?> retType = serviceType.getMethod(invocation.getMethodName(), invocation.getParameterTypes()).getReturnType();
    ...
    public static byte[] post(String url, byte[] requestContent, Map<String, String> headerMap) throws IOException {
        ...
        if (responseCode == HttpStatus.SC_OK || responseCode == HttpStatus.SC_CREATED || responseCode == HttpStatus.SC_ACCEPTED || responseCode == HttpStatus.SC_NO_CONTENT) {
